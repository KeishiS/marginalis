...
- ノート本文・メタデータ・ACL・削除状態の正本は SQLite です。AsciiDoc はノート単位と archive
  の import/export 形式です。
- `server-users` の利用者がログインできます。`server-admins` はすべてのノートを読め、管理できます。
- `/api/v2` が公開 REST API です。仕様は [OpenAPI](docs/openapi-v3.json) を参照してください。
...
`v0.2` の `/api/v1`、ローカル root、ファイル正本、既存データは互換対象ではありません。v3 は空の
...
`MARGINALIS_LISTEN_ADDR`、`OIDC_ISSUER_URL`、`OIDC_CLIENT_ID`、OIDC client secret、
`KANIDM_MEMBERSHIP_API_URL`、read-only Kanidm service-account token が必要です。秘密情報は
