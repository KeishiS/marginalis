...
| [OpenAPI 3.1](docs/openapi.json) | `/api/v1`の機械可読なREST契約。実行中のserverでは`/api/v1/openapi.json`でも取得可能。 |
...
| [リリース手順](docs/release.md) | RCの自動gate、実環境受入、tag・GitHub Releaseの手順 |
...
cargo make test
cargo make verify
# RCまたは正式releaseの前に、NixOS VM testとrelease package buildも実行する。
...
`v0.1.0-rc.1`の範囲はREST API、rootによる保留OIDCユーザー承認、OAuth保護されたMCPである。ブラウザー
