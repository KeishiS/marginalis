...
write_valid_architecture() {
  cat >"$work_directory/architecture.adoc" <<'EOF'
== 一貫して満たすべき設計条件
...
bash "$script_directory/check-design-traceability.sh" \
  "$work_directory/architecture.adoc" "$work_directory/traceability.adoc"
...
if bash "$script_directory/check-design-traceability.sh" \
  "$work_directory/architecture.adoc" "$work_directory/traceability.adoc" >/dev/null 2>&1; then
