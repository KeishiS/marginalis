...
EOF
cat >"$work_dir/traceability.md" <<'EOF'
| 要件 | 自動検証 | 受入 |
| --- | --- | --- |
| REQ-TST-001 | `fixture-test` | 手動確認 |
EOF
bash "$script_dir/check-traceability.sh" \
  "$work_dir/requirements.md" "$work_dir/traceability.md" "$work_dir/no-acceptance"
...
| --- | --- | --- |
| REQ-TST-001 | `fixture-test` | 手動確認 |
| REQ-TST-001 | `duplicate-test` | 手動確認 |
EOF
if bash "$script_dir/check-traceability.sh" \
  "$work_dir/requirements.md" "$work_dir/traceability.md" "$work_dir/no-acceptance" \
  >/dev/null 2>&1; then
