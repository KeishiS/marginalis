...
状態: AdocWeave v0.6.1移行（Issue 029）完了後に着手。
...
AdocWeave v0.6.1の契約とURL・semantic blockの挙動を先に固定しなければ、移行した文書の検証・render結果を
...
4. AdocWeave v0.6.1で安全にrenderできる文書profileを定める。ノート正本に必要なmetadataやACLはrepository
...
- AdocWeave v0.6.1によるparse/renderと内部リンク検証がCIおよびrelease gateで成功する。
