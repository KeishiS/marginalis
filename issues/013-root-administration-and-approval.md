...
状態: 実装中。root login、保留ユーザー承認、無効化、登録policyの永続化、root限定のSQLite監査と
REST APIは実装済み。招待、root login rate limit、専用管理originは後続である。
...
- root login rate limit、ユーザー再有効化、招待、専用管理origin・mTLSは未実装である。
