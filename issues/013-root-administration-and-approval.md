# 013: `root`管理とOIDCユーザー承認

状態: 進行中。`root`ログイン、保留ユーザーの承認と無効化、登録ポリシーの永続化、
`root`限定のSQLite監査、ログインのレート制限およびREST APIは実装済みである。
招待と専用管理オリジンは後続作業とする。
...
- OIDCユーザーの一覧、承認、無効化
- 登録ポリシー（`open`、`approval`）の取得・変更。`invite-only`は招待機能と同時に後続で導入する。
- root管理操作の監査記録
...
- NixOSの`initialRegistrationPolicy`は新規SQLite DBにだけ適用する。既存DBのroot APIによる変更は
  `nixos-rebuild`で上書きしない。
