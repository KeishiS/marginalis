...
状態: 実装中。root login、保留ユーザー承認、無効化、登録policyの永続化とREST APIは実装済み。
監査、招待、root login rate limit、専用管理originは後続である。
...
- 管理操作の永続監査、root login rate limit、ユーザー再有効化、招待、専用管理origin・mTLSは未実装である。
