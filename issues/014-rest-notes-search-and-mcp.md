...
状態: 初期実装済み。結合試験・rate limit・MCP更新操作を後続作業とする。
...
Web UI、ベクトル検索、招待、MCPからのノート更新、rootによるMCP接続は範囲外とする。
...
3. **MCP認証とtransport**
   ...
   - 初期toolは`search_notes`と`get_note`に限定する。各toolはRESTと同じACL、pagination、エラー意味論を用いる。
...
- MCPの`search_notes`と`get_note`が同じuse caseとACL判定を利用する。
- RESTとMCPの検索結果、cursor、削除後の可視性を結合試験で検証する。

## 2026-07-23時点の実装状況

- REST CRUD、ETagを使う条件付き更新・物理削除、ACL非漏洩の一覧・FTS5検索・cursorを実装した。
- MCPの`search_notes`と`get_note`は`NoteUseCases`を共有し、access tokenのscope/resource/利用者状態を
  SQLiteで検証する。
...
- MCPはread/search、構造化create/update、二段階の物理deleteを提供する。rate limitも実装した。
  実clientを使うend-to-end OAuth結合試験と、MCPのリンク一覧toolは未実装である。
