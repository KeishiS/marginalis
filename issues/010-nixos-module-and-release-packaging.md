# 010: NixOSモジュールと公開パッケージ

状態: 初期公開に必要な実装は完了。GitHub公開後の実環境導入とリリース自動化は後続作業である。

## 目的

Marginalisの公開時に、GitHubのNix flake inputからNixOSモジュールを読み込み、
...
1. **早期基盤**: Rustバイナリをpackage化し、flake outputにLinux向けpackageと
   `nixosModules.default`を追加した。
2. **早期基盤**: `services.marginalis`モジュールを実装した。
...
5. **早期基盤**: reverse proxyとの連携例と、運用上の永続パスをREADMEへ追加した。
6. **段階的検証**: モジュール評価とHTTP health endpointのNixOS VM testを追加した。ノート永続化、
...
- `marginalis restore`は完了marker、SQLite integrityおよび全正本を検証して、既存dataDirを変更しない
  新規dataDir候補を作る。NixOS moduleは意図的にin-place restore unitを提供せず、dataDir切替と旧データの
  削除を運用者の明示的判断に残す。
