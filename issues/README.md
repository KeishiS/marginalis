...
`v0.1.0` の受入とリリースは完了した（2026-07-23。[009](009-oidc-provider-registration.md)、
[022](022-v0.1.0-rc.1-release-acceptance.md)）。root 管理は REST API で継続し、管理 UI は
後続とする。既存デプロイの dataDir は移行せず、Issue 015 で明示的に初期化して新しい
スキーマへ移る。
...
2. [029: AdocWeave v0.5.0 への移行](029-adocweave-v0.5.0-migration.md)
...
- AdocWeave の単一`CONTRACT_VERSION`を、解析cache、HTML、Projection、Conformanceおよび
  WASMの再構築判断に使う。
