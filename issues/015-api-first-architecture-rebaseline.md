...
2. **読み取りと書き込みを分離する。** source正本を更新するcommandと、SQLite投影だけを返すqueryを
   分ける。検索は`SearchNotes` queryに集約し、ACLの絞り込み、cursor、順位をその契約へ含める。初期公開の
...
- この再設計では旧`dataDir`を移行しない。新しいSQLite schema、root credential、OIDC identity、
...
- root管理監査はSQLiteへ永続化し、`marginalis rebuild-projections`と`marginalis backup`、対応する
  ...
  oneshotとの排他を確認する。
- NixOS VM上で実OIDCプロバイダーと実MCPクライアントを用いるE2E試験はIssue 030へ移管した。
