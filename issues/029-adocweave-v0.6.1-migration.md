...
- パッケージ版とRust版は、v0.6.1タグの`release-manifest.json`を基準とする。
  必要なRust版は1.97.1である。
- v0.3.0 移行ガイドに残る古いバージョン値が配布目録と異なる場合は、各リリースタグの
...
3. メタデータ検証を`document_attribute_occurrences()`へ移す。重複、設定・解除、値、位置を
   Marginalisの検証型へ変換する。
4. 旧 6 種類のバージョン型をパッケージ版の型へ置き換え、起動時に不一致を拒否する。
...
- 旧 6 種類のバージョン値を削除し、Rust 版と WASM 版の `packageVersion = 0.6.1` 完全一致へ切り替えた。
