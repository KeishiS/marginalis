# 026: OIDCログイン開始要求のブラウザー結合と実行時制限

## 状態

RC.1でログインCSRF対策を完了した。保持期限の設定、信頼するプロキシ、専用管理オリジン、
実行時のリソース上限は未完了である。

## 背景

OIDCの`state`はDB内の一回限りtokenとして検証されるが、ログイン開始したブラウザーとの結合がない。
攻撃者が取得した自身のcode/stateを別のブラウザーに踏ませるログインCSRFを防げない。
...
1. OIDC開始時に、HttpOnlyかつSameSiteなブラウザー用nonce Cookieを発行し、
   `state`と対応するハッシュを保存する。
...
5. 明示的なリソース上限を導入し、公開仕様として文書化する。少なくともノートサイズ、タイトル長、
