...
* ``/api/v3``は公開REST APIです。仕様はlink:docs/openapi.json[OpenAPI]を参照してください。
* MCPはStreamable HTTPで動作し、Authorization Serverを内蔵します。Web UIと同じKanidmで
ログインし、認可codeとtokenはMarginalisが管理します。接続方法は
xref:docs/mcp.adoc#top[MCPへの接続と認可]を参照してください。
* 削除したノートは30日間保管し、日次のNixOSタイマーが期限を過ぎたデータを物理削除します。

== 利用と運用

NixOSの設定、秘密情報、バックアップについてはxref:docs/nixos.adoc#top[NixOSでの運用]、
MCPへの接続方法についてはxref:docs/mcp.adoc#top[MCPへの接続と認可]を参照してください。
...
秘密情報は``++*_++FILE``または実行環境の認証情報を渡す仕組みを使い、Git、Nix store、SQLite、ログへ
保存しないでください。

== 開発

設計上の規則はxref:docs/requirements.adoc#top[要件定義]とxref:docs/architecture.adoc#top[アーキテクチャ]、
今後の作業計画はxref:docs/roadmap.adoc#top[ロードマップ]を参照してください。
...
xref:docs/development.adoc#top[GitHubを使う開発手順]を参照してください。

詳細はxref:docs/README.adoc#top[文書案内]を参照してください。

== ライセンス

Marginalisのソースコードはlink:LICENSE-MIT[MIT License]または
