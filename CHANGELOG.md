...
- 利用経路のないノート単位ACLを廃止し、通常利用者は自身が作成したノートだけを操作できる
  所有者モデルへ単純化した。`server-admins`は従来どおりすべてのノートを管理できる。
- SQLite schemaを4へ、archiveを`marginalis-archive-3`へ更新した。旧databaseと旧archiveの
  移行は提供せず、更新時は旧`dataDir`全体を削除して再初期化する。

### 修正

- REST、MCP、Web UIで所有者認可を共通化し、権限のないノートを一覧から除外して個別操作を
