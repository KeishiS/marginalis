...
マージまでを説明します。リリースの詳しい操作はxref:release.adoc#top[リリース手順]を参照してください。
...
+
coverageの対象と解釈はxref:coverage.adoc#top[本番到達性とカバレッジ]を参照してください。
+
...
条件を直接確認する試験とxref:traceability.adoc#設計条件と検証の対応[要件と検証の対応表]も同じ変更で
更新します。
+
...
v0.5.0以前のローカルIssueとの対応はxref:issue-migration.adoc#top[移行対応表]を参照してください。
...
Pull Requestの必須チェックとxref:release.adoc#top[リリース手順]の検証が成功した後に``main``へマージし、
