...
扱いません。明示的に設定する場合は、これまで同意したscopeの範囲内だけで変更できます。上限を狭めると
範囲外のtoken familyを直ちに失効します。上限を広げてもtokenへscopeを追加しないため、追加した操作には
OAuth再認可が必要です。REST APIでは未設定のクライアントに実効上限、revision `0`、
`scope_ceiling_configured: false`を返します。
