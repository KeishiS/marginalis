...
変更してください。現行のSQLite schema versionは20です。旧versionを起動時に自動移行しません。
...
現行の`migrate-archive`でarchive 17へ変換してから、空のschema 20へ取り込んでください。
この経路では全ノートをAdocWeave 0.33.0の規則で再検証し、題名、タグ、参照索引を再構築します。
...
| --- | --- | --- |
| `marginalis-archive-16` | 0.27.0 | 5 |
...
現行archiveは`marginalis-archive-17`で、AdocWeave package版`0.33.0`とnote profile版`5`を記録します。
...
v0.15.0が作成したarchive 8は、復元前に現行のarchive 17へ変換します。SQLite schema 20には引用の索引、MCP OAuthの状態表、scope上限、認可済みクライアントの管理情報、数式マクロ設定、ノートの作成経路と人手確認記録、書誌情報の取込元と対応情報があるため、
...
archive 8を入力にする場合も、出力はarchive 17です。変換では全ノートをAdocWeave 0.33.0で
再検証します。入力archiveは変更されず、出力先が既に存在する場合は上書きしません。詳しい判断は
[0.33移行判断](adocweave-v0.33-migration.md)を参照してください。

## v0.10.0からの移行

v0.10.0のschema 10には、AdocWeave 0.11.0で導出した題名とタグが保存されています。現行版は
AdocWeave 0.33.0でmetadataを再構築するため、databaseファイルを直接引き継ぎません。
...
3. 現行版の実行ファイルでarchive 7をarchive 17へ変換します。入力と出力には異なる絶対pathを
...
4. 移行が失敗した場合はarchive 17を取り込まず、v0.10.0へ戻ります。改行を残す複数行タグや
   ...
   出力されません。
5. 成功したarchive 17を、次節の手順で空のschema 20へ取り込みます。

CIでは、旧実行環境が作成したschema 9のarchive 7にも同じ移行操作を適用し、入力archiveの不変、
旧archiveの直接取込拒否、schema 20でのノート、ACL、参照、削除状態、revision、数式マクロ設定、来歴と人手確認の一致、archive 17の
