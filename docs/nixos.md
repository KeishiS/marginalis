...
現行の`migrate-archive`でarchive 11へ変換してから、空のschema 12へ取り込んでください。
...
archive 7、8、9、10以外の旧archiveには移行操作を提供しません。
...
現行archiveは`marginalis-archive-11`で、AdocWeave package版`0.20.0`とnote profile版`4`を記録します。
...
v0.16.1が作成したarchive 10、v0.16.0が作成したarchive 9、v0.15.0が作成したarchive 8は、
復元前に現行のarchive 11へ変換します。SQLite schema 12には書誌ライブラリーを追加したため、
...
sudo -u marginalis marginalis migrate-archive \
  ...
  --output /srv/marginalis-migration/archive-11.json
sudo -u marginalis marginalis verify-restore \
  --input /srv/marginalis-migration/archive-11.json
```

archive 8を入力にする場合も、出力はarchive 11です。変換では全ノートをAdocWeave 0.20.0で
...
3. 現行版の実行ファイルでarchive 7をarchive 11へ変換します。入力と出力には異なる絶対pathを
...
4. 移行が失敗した場合はarchive 11を取り込まず、v0.10.0へ戻ります。改行を残す複数行タグや
   ...
   出力されません。
5. 成功したarchive 11を、次節の手順で空のschema 12へ取り込みます。

CIでは、旧実行環境が作成したschema 9のarchive 7にも同じ移行操作を適用し、入力archiveの不変、
旧archiveの直接取込拒否、schema 12でのノート、ACL、参照、削除状態、revisionの一致、archive 11の
