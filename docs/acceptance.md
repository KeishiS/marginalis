...
- Kanidm、TLS、サブパス、OIDC、MCP OAuth、バックアップ、削除、障害診断
...
2. 編集中に未保存、保存中、保存成功、保存失敗が区別されることを確認します。位置付きの入力診断
   から文書の該当箇所へ移動でき、プレビュー更新の失敗後も最後に成功した表示が残ることを
   確認します。
3. revision競合で編集開始時点、編集中、現在保存済みの内容を比較し、最新revisionへ再保存します。
4. 目次、表、引用、リスト、コード、インライン数式、ブロック数式が判読でき、横幅を超える表、
   コード、数式を内容ごとにスクロールできることを確認します。
