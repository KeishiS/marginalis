...
6. CSL-JSONファイルを事前確認し、新規項目を取り込みます。同じ取込元を再度指定して、外部だけを
...
7. 所有者が閲覧者と編集者をACLへ追加し、それぞれの表示と操作範囲を確認します。共有相手の画面でも
   引用が作成者のライブラリーで解決した表示になることを確認します。
8. 所有するノートを閲覧画面から削除し、通常の一覧から消えて削除済みノートの画面に現れることを
   確認します。題名、削除日時、完全削除予定、revisionを確認して復元し、通常の一覧と閲覧画面へ
   戻ることを確認します。共有相手には削除操作と削除済みノートの情報が表示されないことも確認します。
9. 数式マクロ設定へ`argmax`と`bm`の定義例を追加し、`\argmax_{x \in S} f(x)`と`\bm{x}`が
   所有するノートの閲覧とプレビューで組版されることを確認します。共有相手の画面でも所有者の
   定義が使われることを確認します。
10. Web UI、汎用REST API、MCPからそれぞれノートを作成し、作成経路が`web`、`rest`、`mcp`として
   表示されることを確認します。所有するノートを確認済みにした後は`reviewed`となり、本文またはACLを
   更新すると`pending`へ戻ること、共有相手には確認者の識別情報が表示されないことも確認します。
11. 対象外利用者にノートと関連情報が表示されず、直接指定した操作も`404`になることを確認します。
12. ChatGPT、Claude Code、Codex CLIからMCPへ接続し、RESTと同じ閲覧・更新結果になることを
...
13. MCPクライアントの接続解除でrefresh tokenとauthorization grantを取り消し、再認可なしでは
   refreshできないこと、既発行access tokenが拒否されるまでの時間を確認します。
14. アーカイブを本番から隔離した空のデータベースへ復元し、ノート、ACL、参照、削除状態、revision、
    作成経路、人手確認記録、書誌ライブラリー、取込元と対応情報、数式マクロ設定が一致することを
    確認します。
15. `export-documents`で書き出した文書書庫を変更せずに空のデータベースへ取り込み、revisionと
...
- [v0.33.0](acceptance-results/v0.33.0.md)
- [v0.32.0](acceptance-results/v0.32.0.md)
- [v0.31.0](acceptance-results/v0.31.0.md)
