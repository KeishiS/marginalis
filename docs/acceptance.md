...
| 対象 | 現在の自動検証 | この手順で確認すること |
| --- | --- | --- |
| REST・MCP | プロセス内のAxum、試験用OIDC provider、SQLiteを使った認証・CRUD・検索・OAuth | 本番のKanidm、リバースプロキシ、実際に使用するMCPクライアントとの接続 |
| NixOS | モジュール評価、サービス・保守oneshot・root-only縮退起動のVM試験 | 本番設定での起動、TLS・サブパスを含む到達性、タイマーの稼働 |
| データ保護 | バックアップ・復元候補・投影再構築の単体および結合試験 | 永続保存先へのバックアップ、復元候補の内容、運用手順からの復旧 |
| ブラウザー | `/acceptance`のHTML、CSRF、CSP、サブパスURLのプロセス内試験 | 実ブラウザーでのOIDCログインとフォーム操作 |

リバースプロキシと実ブラウザーを含む自動化、実Kanidmを使う認証試験、失敗時artifactの収集は
[Issue 030](../issues/030-end-to-end-test-automation-readiness.md)で進めます。自動化された項目も、
本番固有の設定や外部サービスとの接続を変更したリリースでは、この手順で再確認します。
...
状態変更フォームでは、画面に埋め込まれたセッション連動 CSRF トークンを hidden field で
送信します。通常の HTML フォームは任意の HTTP ヘッダーを設定できないため、この画面には
REST API の `Origin`・`Sec-Fetch-Site` 要件を適用しません。セッション Cookie の
`SameSite=Lax` とサーバー側の CSRF トークン照合は引き続き有効です。
