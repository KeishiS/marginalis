...
| REQ-API-005 | ノート・書誌scope対応、scope上限の共通部分・revision・選択的失効の単体試験、同意scopeの部分選択・フォーム改変拒否のHTTP試験、認可済みclientの本人境界・最終利用・上限変更のSQLite試験、REST・React設定画面試験、初回・段階的scope challengeのprotocol版別結合試験、metadata契約試験 | MCP権限照合、同意scopeの選択、client別上限変更後の再認可、段階的scope追加 |
| REQ-API-006 | RFC 7009・token family失効試験、認可取消APIのHTTP契約試験、認可済みclient管理画面のReact試験と本番到達性検査 | client接続解除、即時失効 |
...
| ARCH-OAUTH-003 | `marginalis-application::mcp_oauth::tests::unset_scope_ceiling_uses_every_supported_scope_without_becoming_configured`、`configured_scope_ceiling_preserves_an_empty_or_partial_setting`、`marginalis-sqlite::tests::oauth::client_id_metadata_document_clients_complete_the_authorization_flow`、`client_authorizations_are_owner_scoped_and_record_use_and_revocation`、`marginalis-web::http::tests::rest_notes::owner_can_list_and_restrict_their_mcp_client_authorizations`、`McpAccessSettingsPage.test.tsx` |
