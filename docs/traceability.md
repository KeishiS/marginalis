...
| REQ-API-005 | ノート・書誌scope対応、scope上限の共通部分・revision・選択的失効の単体試験、同意scopeの部分選択・フォーム改変拒否のHTTP試験、認可済みclientの本人境界・最終利用・上限変更のSQLite試験、REST・React設定画面試験、MCP scope結合試験、metadata契約試験 | MCP権限照合、同意scopeの選択、client別上限変更後の再認可 |
