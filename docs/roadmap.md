...
`v0.18.0`では、利用者ごとのCSL-JSON書誌ライブラリーを追加します。Web UIとREST APIでは
書誌情報の検索、登録、編集、削除を提供し、MCPでは検索、1件または最大100件の登録、削除を
提供します。引用の組版はAdocWeave側の`cite:`対応後に追加するため、引き続き
[#147](https://github.com/KeishiS/marginalis/issues/147)で管理します。
