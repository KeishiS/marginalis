...
`v0.18.0`は2026-07-30に公開しました。利用者ごとのCSL-JSON書誌ライブラリーを追加し、Web UIと
REST APIでは書誌情報の検索、登録、編集、削除、MCPでは検索、1件または最大100件の登録、削除を
提供します。

`v0.19.0`は公開準備中です。AdocWeave 0.23.0への更新に合わせて本文の`cite:[key]`を書誌
ライブラリーで解決し、引用表示と参考文献一覧を組み立てます。解決にノート作成者のライブラリーを
使う理由は
[引用はノート作成者の書誌ライブラリーで解決する](adr/0006-引用はノート作成者の書誌ライブラリーで解決する.md)、
版別の確認結果は[v0.19.0受入結果](acceptance-results/v0.19.0.md)へ記録します。
