...
[#198](https://github.com/KeishiS/marginalis/issues/198)では、閲覧できるノートと、その間の参照を
図として表示します。点は閲覧できるノートだけ、線は始点と終点の両方が閲覧できる場合だけとし、
閲覧できないノートの存在と件数を開示しません。図を情報へ至る唯一の手段にせず、同じ内容を
キーボードと支援技術で扱える一覧としても提供します。
...
本文の手掛かりからノートを再発見する手段は、当面提供しません。探す手段は一覧のタグと更新日に
よる絞り込みだけです。

検索方式を比較する#19は、意味検索と埋め込みモデルを比較対象に含み、機械学習を伴う作業へ進む
前提になっていたため終了しました。必要になった時点で、固定した評価集合の作成から改めて
起こします。
