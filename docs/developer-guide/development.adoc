...
対応を検査します。アーキテクチャの「一貫して満たすべき設計条件」を変更する場合は、
...
+
rulesetはsquash方式とmerge方式を許可しますが、上の理由からsquash方式を使用します。
merge方式を選ぶと``main``に分岐が生じ、以降の履歴を一列で読めなくなります。
+
...
xref:release.adoc#top[リリース手順]を正本とします。``v0.2.0``の正式リリース後はリリース候補版を
公開せず、``v0.x.y``形式の通常版だけを公開します。
