...
決める。WebとMCPの同意画面は、同じ``(issuer, subject)``を所有者identityに使用する。
...
採用した保存基盤の移行計画です。\#548のSQLite前進migration基盤は実装済みです。
principalへの切替はまだ実装していないため、\#549と\#550が完了するまでは、上の
``ARCH-IDENTITY-001``、``ARCH-AUTHZ-001``と xref:requirements.adoc#top[現行要件]を現在の動作として
優先します。
...
引き継ぎとaliasのarchive往復です。外部identityと``PrincipalId``を認可条件として混在させる中間schemaは
公開しません。その後、\#541のノート版履歴と\#540の添付を``PrincipalId``へ直接結び付けます。
