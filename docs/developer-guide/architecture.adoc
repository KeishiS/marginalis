...
採用した保存基盤の移行計画です。\#548のSQLite前進migration基盤と、\#549のschema 23および
``PrincipalId``への切替は実装済みです。identityの引き継ぎを行う管理commandと、alias群を失わない
archive契約は\#550で追加します。
...
引き継ぎとaliasのarchive往復です。\#549までの実装では、schema 22に現れるidentityを推測でまとめず、
完全に一致する組ごとに一つのprincipalへ移しています。その後、\#541のノート版履歴と\#540の添付を
