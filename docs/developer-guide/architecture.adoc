...
``marginalis-auth-oidc``は外部identity provider portの薄いadapterです。OIDC discovery・
code exchange・ID token検証の本体は、共有crate ``oidc-browser-login``が担当します(ADR 0015)。
設定した許可claim値との照合による利用可否の判断はapplicationが担当します。
...
├── marginalis-auth-oidc       OIDCログインadapter（共有crateへの写像）
