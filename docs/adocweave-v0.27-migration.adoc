...
0.27.0までの4版分をまとめて取り込みます。現行の入力方法はxref:rest-api.adoc#top[REST API]、archiveの
変換手順はxref:nixos.adoc#top[NixOSでの運用]を参照してください。

前回の更新はxref:adocweave-v0.23-migration.adoc#top[AdocWeave 0.23移行判断]にあります。
