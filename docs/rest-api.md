...
`GET /api/v1/search?q=<query>&limit=50&cursor=...`は、可視なノートだけをSQLite FTS5で検索する。各結果は
`note_id`と`title`を返す。一致箇所の抜粋や本文は返さない。検索語はFTS構文として解釈せず、空白で区切った
各語をAND条件として扱う。一覧・検索とも応答は`notes`と`next_cursor`を持つobjectである。
...
`next_cursor`だけを受理し、`limit`の既定値は50、上限は100である。現在のcursorはoffsetを不透明化した値のため、
ページ取得中に一覧の並びが変化すると重複または欠落することがある。作成者・参照方向filterは公開していない。
