...
現在のリリースでは、一般利用者向けの Web UI を提供していません。この REST API が利用者向けの
主要な境界です。同一オリジンの `/acceptance` は受入確認専用の画面であり、ノートの閲覧・編集 UI
ではありません。
...
`/api/v1/notes` 以下の操作には、OIDC ログインまたは root ログインで発行される
...
`POST`・`PUT`・ログアウトなど Cookie を伴う変更操作では、次の 3 点をすべて満たす必要が
あります。満たさないリクエストは `403` で拒否します。

1. `X-CSRF-Token` ヘッダーに `marginalis_csrf` と同じ値を付ける。
2. `Origin` ヘッダーが公開ベース URL のオリジンと完全に一致する。
3. `Sec-Fetch-Site` ヘッダーが `same-origin` または `none` である。

リバースプロキシが付与する `X-Forwarded-*` はこの判定に使いません。ブラウザ以外の API
クライアントも、同じヘッダーを明示的に送ってください。セッションを持たない
...
返します。REST クライアントはこのドキュメントを契約として扱ってください。Cookie・CSRF・
`Origin`・Fetch Metadata は HTTP 固有のセキュリティ機構であり、MCP のツール契約には
含まれません。

`v0.1.0-rc.2` では、この OpenAPI ドキュメントを v0.1.0 の凍結候補としています。RC 期間中に
フィールド・ステータス・ヘッダー・エンドポイントの破壊的変更が許されるのは、セキュリティ、
データ破損、ACL 漏洩、相互運用性に関わるリリースブロッカーの修正だけです。v0.1.0 で凍結した
後の破壊的変更は新しいバージョンパスで行い、既存バージョンには少なくとも 1 リリース周期の
非推奨告知と移行手順を用意します。
...
root のパスワードを HTTP リクエストボディ以外の場所に記録・保存しないでください。
...
`POST /auth/logout` は現在のセッションをサーバー側で失効させ、`marginalis_session` Cookie を
削除して `204` を返します。
