...
`v0.1.0-rc.2`は、研究室内で REST API と OAuth 保護 MCP を運用する候補版である。
通常利用者向けWeb UI、SMTP、招待、ユーザー再有効化、グループ ACL、専用管理 origin・mTLSは含めない。
実環境受入専用の`/acceptance`は含むが、製品のWeb UIではない。

API pathは`/api/v1`、data formatはv1である。RC.2で公開する[OpenAPI contract](openapi.json)は
...
1. `Cargo.toml`群とflakeのversionが`0.1.0-rc.2`であること、`LICENSE-MIT`、`LICENSE-APACHE`および
   `CHANGELOG.md`が確定していることを確認する。
2. annotated tag `v0.1.0-rc.2`を作成してpushする。
3. GitHub Actionsの`Release gate`成功後、公開配布が必要な場合だけGitHub Releaseをprereleaseとして作成する。
   tagだけでの候補版運用ではRelease作成を省略できる。
