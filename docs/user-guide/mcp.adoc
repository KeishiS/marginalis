...
MCPの初期化応答と``add_bibliography_item``のtool説明にも、一括取込が非対応であることと、代わりに
Web UIまたはREST APIを使うことを示します。クライアントは、存在しない一括取込toolを探したり、競合解決を
推測して複数回の追加へ置き換えたりせず、この案内を利用者へ伝えてください。
