...
この経路では全ノートをAdocWeave 0.40.1の規則で再検証し、題名、タグ、参照索引を再構築します。
...
現行archiveは``marginalis-archive-17``で、AdocWeave package版``0.40.1``とnote profile版``5``を記録します。
...
``0.36.0``で、現行の``0.40.1``と違うためです。archiveの構造は同じでも、本文を現行の規則で
再検証してから取り込みます。
...
archive 8を入力にする場合も、出力はarchive 17です。変換では全ノートをAdocWeave 0.40.1で
...
AdocWeave 0.40.1でmetadataを再構築するため、databaseファイルを直接引き継ぎません。
