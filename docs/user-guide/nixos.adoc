...
変更してください。現行のSQLite schema versionは21です。旧versionを起動時に自動移行しません。

schema 20から更新する場合は、v0.36.0の実行環境で``export-archive``を実行し、archive 17を現行の
``migrate-archive``で再検証してから、空のschema 21へ``import-archive``してください。同期cursorと変更索引、
...
現行の``migrate-archive``でarchive 17へ変換してから、空のschema 21へ取り込んでください。
...
再検証してから取り込みます。
SQLite schema 21には引用の索引、MCP OAuthの状態表、scope上限、同期cursorと変更索引、
認可済みクライアントの管理情報、数式マクロ設定、ノートの作成経路と人手確認記録、書誌情報の取込元と対応情報があるため、
