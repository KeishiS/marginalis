...
その書庫を現行の``restore-archive``で空のschema 22へ取り込んでください。``restore-archive``は
...
現在のschemaは22で、未適用のmigrationはありません。その場合は成功logの
``applied_migrations=0``と``backup=none``で何も変更しなかったことを示し、SQLite退避も作りません。
