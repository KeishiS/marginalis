...
クライアントを接続し直します。v0.31.0には文献情報の取込元と対応情報がないため、移行後に最初に
...
この経路では全ノートをAdocWeave 0.41.0の規則で再検証し、題名、タグ、参照索引を再構築します。
...
``0.36.0``で、現行の``0.41.0``と違うためです。archiveの構造は同じでも、本文を現行の規則で
再検証してから取り込みます。
SQLite schema 22には引用の索引、MCP OAuthの状態表、scope上限、同期cursorと変更索引、
認可済みクライアントの管理情報、数式マクロ設定、ノートの作成経路と人手確認記録、文献情報の取込元と対応情報、
...
archive 8を入力にする場合も、出力はarchive 17です。変換では全ノートをAdocWeave 0.41.0で
...
AdocWeave 0.41.0でmetadataを再構築するため、databaseファイルを直接引き継ぎません。
