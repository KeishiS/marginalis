...
この経路では全ノートをAdocWeave 0.36.0の規則で再検証し、題名、タグ、参照索引を再構築します。
...
現行archiveは``marginalis-archive-17``で、AdocWeave package版``0.36.0``とnote profile版``5``を記録します。
...
v0.33.0からv0.35.1までが作成したarchive 17、v0.18.0が作成したarchive 11、v0.16.1が作成したarchive 10、
v0.16.0が作成したarchive 9、v0.15.0が作成したarchive 8は、復元前に現行のarchive 17へ変換します。
形式名が同じarchive 17を変換するのは、記録しているAdocWeave package版が``0.33.0``で、現行の
``0.36.0``と違うためです。archiveの構造は同じでも、本文を現行の規則で再検証してから取り込みます。
SQLite schema 20には引用の索引、MCP OAuthの状態表、scope上限、
...
archive 8を入力にする場合も、出力はarchive 17です。変換では全ノートをAdocWeave 0.36.0で
...
AdocWeave 0.36.0でmetadataを再構築するため、databaseファイルを直接引き継ぎません。
...
出力されません。
. 成功したarchive 17を、次節の手順で空のschema 20へ取り込みます。
