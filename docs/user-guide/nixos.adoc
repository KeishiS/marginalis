...
取り込みの互換は、**直近5マイナー世代のリリースが書き出した書庫**に限定しています(設計判断は
ADR 0017を参照)。リリースのたびに、5世代より前へ落ちた契約を受け入れ対象と本節から削除します。

``migrate-archive``が受け取れるのは、次の組で書き出された書庫だけです。形式、AdocWeave package版、
note profile版の3つがすべて一致する必要があります。
...
現行のv0.46.0が書き出す書庫(AdocWeave package版0.41.0)は、変換なしでそのまま
``import-archive``できます。形式名はarchiveの構造を表し、解析器の版はmanifestの
``adocweave_package_version``が表します。形式名が同じでも、記録されたAdocWeave package版が
現行と違う書庫は直接取り込めないため、``migrate-archive``で変換してから取り込みます。
...
上の一覧の旧契約なら変換したうえで、隔離databaseでの復元一致を検証してから
...
上の一覧より古い書庫を持ち込む場合は、その書庫へ対応していた過去のリリース
(https://github.com/KeishiS/marginalis/releases[GitHub Releases]から取得)を一時的な環境で使い、
``migrate-archive``で当時の現行形式へ変換してから、この一覧に入る形で書き出し直して持ち上げます。
たとえばv0.8.0が書き出したarchive 7は、v0.45.0の``migrate-archive``で一覧内の組
(``marginalis-archive-17``、AdocWeave 0.40.1)へ変換できます。
