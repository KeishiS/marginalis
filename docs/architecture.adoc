...
説明します。利用者向けの機能はxref:requirements.adoc#top[現行要件]、HTTPの入出力は
...
``ES256``だけを許可する。別の署名方式を追加する場合はxref:security.adoc#top[セキュリティ]の依存脆弱性判断を
先に更新する。
...
ログへ渡しません。安定したeventとfieldの契約はxref:observability.adoc#top[ログと障害診断]を正とし、
...
設計を確定した経緯はxref:v0.3.0-design.adoc#top[再設計判断記録]を参照してください。
