...
xref:adocweave-v0.23-migration.adoc#top[0.23移行判断]で取り消しました。現在は``cite:``を受理し、
書誌ライブラリーで解決して表示します。
____
...
扱いを決めた理由を記録します。現行の入力方法は xref:rest-api.adoc#top[REST API]、archiveの変換手順は
...
*理由*: 解決した書誌情報をHTMLの描画へ渡す経路がAdocWeave側にまだありません。関連する課題は
AdocWeave issue \#313と\#314です。この状態で受理すると、利用者には解決されないcitation keyがそのまま表示されるか、
``unresolved_references``の設定によっては何も表示されないノートができます。後から解決経路が
整った時点で表示が変わるため、その途中状態を保存させないことを選びました。

*代替手段*: 従来どおり標準のbibliographyを使用できます。これは``[[[key]]]``で定義し、
``<<key>>``で参照する書き方です。この記法の解析は0.22.0でも変わっていません。

*解除の条件*: AdocWeave側で解決済みの引用をHTMLへ渡せるようになった時点で、この拒否を外し、
xref:requirements.adoc#top[書誌ライブラリー]の項目と結び付けます。追跡はMarginalis issue \#147で行います。
