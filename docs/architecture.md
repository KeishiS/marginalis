...
- Cookie セッションを伴う変更操作では、CSRF トークンと起動時に固定した公開オリジンを
  検証する。`Sec-Fetch-Site` がある場合は追加で検証する。`X-Forwarded-*` は、この判定にも
  `root` ログインの補助レート制限にも使わない。`/acceptance` のサーバー生成 HTML フォーム
  だけは、hidden field のセッション連動 CSRF トークンを照合し、オリジンヘッダーを要求しない。
