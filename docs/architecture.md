...
戻りlinkはAdocWeaveが生成するため、MarginalisはHTMLを組み立てません。生成した一覧は保存する本文へ
書き込みません。書誌情報の文字列は表示であって記法ではないため、文書adapterがAsciiDocの記法として
解釈されない形へ直します。
...
`marginalis-contract/src/typescript-contracts.ts`を正本として
