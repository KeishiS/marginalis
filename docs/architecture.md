...
閲覧画面の直接参照一覧は、保存済み本文から得た参照先IDと、その時点で可視なノートを突き合わせる。
これにより本文更新、権限変更、ソフトデリート、復元、物理削除へ直ちに追従し、不可視なノートの
存在や件数を投影データから漏らさない。ノート数の増加により読み取り費用が問題になった場合は、
同じ認可条件を保った参照投影を別途導入する。
